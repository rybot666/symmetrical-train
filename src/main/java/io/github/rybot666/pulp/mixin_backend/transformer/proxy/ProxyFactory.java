...
public class ProxyFactory {
    ...
    static final WeakHashMap<String, Class<? extends ProxyMarker>> PROXY_CLASS_MAP = new WeakHashMap<>();
    ...
    public static ClassNode generateBaseProxy(ClassNode target) {
        ...
        // generate methods for getting the proxy associated with a target object
        //
        // generates a generic `static Proxy getAssociatedProxy(Target $this)` that invokes `PulpBootstrapMethods#getProxy`  via indy
        proxy.methods.add(Util.make(new MethodNode(), m -> {
            m.name = "getAssociatedProxy";
            m.access = Opcodes.ACC_STATIC | Opcodes.ACC_PUBLIC;
            m.desc = Type.getMethodDescriptor(
                    Type.getObjectType(proxy.name),
                    Type.getObjectType(target.name)
            );
            ...
            Handle handle = new Handle(
                    ...
                    Type.getInternalName(PulpBootstrapMethods.class),
                    "getProxy",
